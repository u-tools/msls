...
#include <malloc.h> // AEK for alloca()
...
#include <sys/stat.h> // AEK, must preceed dirent.h
...
#else
# if WIN32
#  define REAL_DIR_ENTRY(dp) 1  // not always present on WIN32 (e.g., ls --fast netdrive)
# else
...
#include <mbstring.h> // AEK
#include "system.h" // AEK for S_ISDIR

//#include "filecntl.h" // AEK
#if !defined (F_OK)
#  define F_OK 0
#endif
...
#pragma warning(disable: 4131) // ignore old-style C decls
#pragma warning(disable: 4701) // ignore use before set
     ...
     char *dir; // AEK
{
  ...
  if (dir != NULL) {
    //
    // Concat dir + pattern to check for share path
    //
    buf = (char *) alloca(strlen(dir) + strlen(pattern) + 1);
    strcpy(buf, dir);
    strcat(buf, pattern);
    pattern = buf;
  }
  ...
  if (isalpha(p[0]) && p[1] == ':') {
    p += 2; // skip initial "C:"
  } else if (p[0] == '/' && p[1] == '/') { // if "//server"
    if ((p = strchr(p+2, '/')) == NULL) // not "//server/share"
      return 0; // "//server" by itself cannot be case-matched
    if ((p = strchr(p+1, '/')) == NULL) // not "//server/share/"
      return 0; // "//server/share" by itself cannot be case-matched
    ++p;
  }
  while ((c = *p++) != '\0')
    if (isalpha((unsigned int)c)) {  // locale-aware
      break;
    }
  if (c != '\0') // if has alphabetics
    return 1; // glob to set case
#endif
  return (0);
}
...
#ifdef WIN32
      d = opendir_with_pat (dir,pat,TRUE/*bCache*/); // pass pat for network speedup - AEK
#else
      d = opendir (dir);
#endif
      if (d == NULL)
...
#undef dname
  ...
  //for (i = 0; i < count; ++i)
  for (i = count-1; (int)i >= 0; --i) // reverse back to A-Z order - AEK
    {
     ...
     char *dir, **array;
{
  ...
  // BUG: "c:" + "foo" -> "c:foo", not "c:/foo" !!!
  //
  if ((int)l == FILESYSTEM_PREFIX_LEN(dir)) // BUGFIX - AEK
    add_slash = 0; // BUGFIX
     ...
     char *pathname;
{
  ...
  //
  // Special case for Windows filesystems: "c:foo" -> "c:","foo" // AEK
  //
  if (filename == NULL && FILESYSTEM_PREFIX_LEN(pathname) != 0) {
    filename = &pathname[FILESYSTEM_PREFIX_LEN(pathname)-1];
  }
#endif
  if (filename == NULL)
    {
  ...
  if (glob_pattern_p (directory_name, NULL/*AEK*/))
    {
      char **directories;
      register unsigned int i;
